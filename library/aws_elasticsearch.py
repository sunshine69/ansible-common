...
## This source file has been modified by Steve Kieu <steve.kieu@xvt.com.au> to
# Add deletion support
# Split function of oprations rather than in one function main
...
def main():
    ...
    invocations = {
        "present": create_elasticsearch_domain,
        "absent": delete_elasticsearch_domain
    }
...
def create_elasticsearch_domain(client, module):
    ...
    return (changed, "Domain created or updated")

def delete_elasticsearch_domain(client, module):
    changed = False
    name = module.params.get('name')
    try:
        response = client.describe_elasticsearch_domain(DomainName=name)
    except botocore.exceptions.ClientError as e:
        if e.response['Error']['Code'] == 'ResourceNotFoundException':
    ...
    if response['DomainStatus']:
        ...
        result = client.describe_elasticsearch_domain_config(DomainName=name)
